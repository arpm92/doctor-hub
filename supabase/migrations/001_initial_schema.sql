...
BEGIN
    SELECT json_build_object(
        ...
        'pending_doctors', (SELECT COUNT(*) FROM public.doctors WHERE status = 'pending'),
        'approved_doctors', (SELECT COUNT(*) FROM public.doctors WHERE status = 'approved'),
        'rejected_doctors', (SELECT COUNT(*) FROM public.doctors WHERE status = 'rejected'),
        'suspended_doctors', (SELECT COUNT(*) FROM public.doctors WHERE status = 'suspended'),
        'recent_registrations', (SELECT COUNT(*) FROM public.doctors WHERE created_at >= now() - interval '30 days'),
        'specialties', COALESCE((SELECT json_object_agg(specialty, count) FROM (SELECT specialty, COUNT(*) as count FROM public.doctors GROUP BY specialty) as s), '{}'::json)
    ) INTO stats_result;
    RETURN stats_result;
END;
$$;
...
DROP POLICY IF EXISTS "Allow public read access to approved doctors" ON public.doctors;
CREATE POLICY "Allow public read access to approved doctors" ON public.doctors FOR SELECT USING (status = 'approved');

DROP POLICY IF EXISTS "Allow doctor to see their own profile" ON public.doctors;
CREATE POLICY "Allow doctor to see their own profile" ON public.doctors FOR SELECT USING (auth.uid() = id);

DROP POLICY IF EXISTS "Allow doctor to update their own profile" ON public.doctors;
CREATE POLICY "Allow doctor to update their own profile" ON public.doctors FOR UPDATE USING (auth.uid() = id);

DROP POLICY IF EXISTS "Allow system to insert doctor profiles" ON public.doctors;
CREATE POLICY "Allow system to insert doctor profiles" ON public.doctors FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow admins to manage all doctor records" ON public.doctors;
CREATE POLICY "Allow admins to manage all doctor records" ON public.doctors FOR ALL USING ( 
    EXISTS (SELECT 1 FROM public.admins WHERE id = auth.uid()) 
);

-- RLS Policies for Patients
DROP POLICY IF EXISTS "Allow patient to manage their own data" ON public.patients;
CREATE POLICY "Allow patient to manage their own data" ON public.patients FOR ALL USING (auth.uid() = id);

DROP POLICY IF EXISTS "Allow system to insert patient profiles" ON public.patients;
CREATE POLICY "Allow system to insert patient profiles" ON public.patients FOR INSERT WITH CHECK (true);

-- RLS Policies for Admins
DROP POLICY IF EXISTS "Allow admin to see their own record" ON public.admins;
CREATE POLICY "Allow admin to see their own record" ON public.admins FOR SELECT USING (auth.uid() = id);

DROP POLICY IF EXISTS "Allow admin to update their own record" ON public.admins;
CREATE POLICY "Allow admin to update their own record" ON public.admins FOR UPDATE USING (auth.uid() = id);

DROP POLICY IF EXISTS "Allow system to insert admin profiles" ON public.admins;
CREATE POLICY "Allow system to insert admin profiles" ON public.admins FOR INSERT WITH CHECK (true);

-- RLS Policies for Locations
DROP POLICY IF EXISTS "Allow public read access to locations" ON public.doctor_locations;
CREATE POLICY "Allow public read access to locations" ON public.doctor_locations FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow doctor to manage their own locations" ON public.doctor_locations;
...
DROP POLICY IF EXISTS "Allow public read access to published articles" ON public.doctor_articles;
CREATE POLICY "Allow public read access to published articles" ON public.doctor_articles FOR SELECT USING (status = 'published');

DROP POLICY IF EXISTS "Allow doctor to manage their own articles" ON public.doctor_articles;
